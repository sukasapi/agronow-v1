...
class Classroom extends CI_Controller {
	...
	function evaluasi_lv3($classroom_id=NULL){
		...
		$post = $this->input->post();
		if(!empty($post)) {
			$tahun_evaluasi = (int) $post['tahun_evaluasi'];
	...
	function evaluasi_lv3_peserta($classroom_id=NULL){
		...
		$ui = '';
		$sql = "select m.member_id, m.member_name, m.member_nip, cm.crm_step from _classroom_member cm, _member m where cm.member_id=m.member_id and m.member_status='active' and cr_id='".$classroom_id."' order by m.member_name ";
		$res = $this->db->query($sql);
		$row = $res->result_array();
					...
					if(!empty($data_atasan)) {
						...
						if(!empty($result)) {					
							...
							if(is_null($result[0]['company_code'])) {
								...
								$sqlC = "select group_id from _group where aghris_company_code='".$result[0]['company_code']."' ";
	...
	function evaluasi_lv3_do_rekap($kat=NULL,$did=NULL){
		...
		create_log($this->section_id,$classroom_id,'Rekap','Evaluasi Pelatihan Lv.3');
		
		if ($this->db->trans_status() === FALSE) {
			create_log($this->section_id,$classroom_id,'Gagal Rekap','Evaluasi Pelatihan Lv.3');
	...
	function evaluasi_lv3_do_export($kat=NULL,$did=NULL){
		...
		$sheet->setCellValue('F2', 'Nilai Pre');
		$sheet->mergeCells('G2:G3');
		$sheet->setCellValue('G2', 'Nilai Post');
		$sheet->mergeCells('H2:P2');
		$sheet->setCellValue('H2', 'Atasan (Pre)');
		$sheet->mergeCells('Q2:X2');
		$sheet->setCellValue('Q2', 'Kolega (Pre)');
		$sheet->mergeCells('Y2:AG2');
		$sheet->setCellValue('Y2', 'Atasan (Post)');
		$sheet->mergeCells('AH2:AO2');
		$sheet->setCellValue('AH2', 'Kolega (Post)');
		// baris 3 - pre
		...
		$sheet->setCellValue('Z3', 'knowledge');
		$sheet->setCellValue('AA3', 'rerata skill');
		$sheet->setCellValue('AB3', 'rerata attitude');
		$sheet->setCellValue('AC3', 'rerata ksa');
		$sheet->setCellValue('AD3', 'ksa x bobot');
		$sheet->setCellValue('AE3', 'rerata behaviour');
		$sheet->setCellValue('AF3', 'behaviour x bobot');
		$sheet->setCellValue('AG3', 'nilai x bobot');
		$sheet->setCellValue('AH3', 'knowledge');
		$sheet->setCellValue('AI3', 'rerata skill');
		$sheet->setCellValue('AJ3', 'rerata attitude');
		$sheet->setCellValue('AK3', 'rerata ksa');
		$sheet->setCellValue('AL3', 'ksa x bobot');
		$sheet->setCellValue('AM3', 'rerata behaviour');
		$sheet->setCellValue('AN3', 'behaviour x bobot');
		$sheet->setCellValue('AO3', 'nilai x bobot');

		// sql utama
		$i = 0;
		$x = 3;
		$sqlU =
			"select c.cr_id, c.cr_name
			 from _classroom_evaluasi_lv3_header h, _classroom c
			 where h.cr_id=c.cr_id ".$addSql." order by c.cr_name";
		$resU = $this->db->query($sqlU);
		$rowU = $resU->result_array();
		foreach($rowU as $keyU => $valU) {
			$classroom_id = $valU['cr_id'];
			...
			foreach($rowC as $keyC => $valC) {
				...
				$sheet->setCellValue('C'.$x, $classroom_name);
				$sheet->setCellValue('D'.$x, $valC['member_nip']);
				$sheet->setCellValue('E'.$x, $valC['member_name']);
				$sheet->setCellValue('F'.$x, $valC['nilai_pre_test']);
				$sheet->setCellValue('G'.$x, $valC['nilai_post_test']);
				...
				$sheet->setCellValue('Z'.$x, $nilai_detail['detail']['post']['atasan']['nilai_k_average']);
				$sheet->setCellValue('AA'.$x, $nilai_detail['detail']['post']['atasan']['nilai_s_average']);
				$sheet->setCellValue('AB'.$x, $nilai_detail['detail']['post']['atasan']['nilai_a_average']);
				$sheet->setCellValue('AC'.$x, $nilai_detail['detail']['post']['atasan']['nilai_ksa_average']);
				$sheet->setCellValue('AD'.$x, $nilai_detail['detail']['post']['atasan']['nilai_ksa_x_bobot']);
				$sheet->setCellValue('AE'.$x, $nilai_detail['detail']['post']['atasan']['nilai_b_average']);
				$sheet->setCellValue('AF'.$x, $nilai_detail['detail']['post']['atasan']['nilai_b_x_bobot']);
				$sheet->setCellValue('AG'.$x, $nilai_detail['detail']['post']['atasan']['nilai_x_bobot']);
				$sheet->setCellValue('AH'.$x, $nilai_detail['detail']['post']['kolega']['nilai_k_average']);
				$sheet->setCellValue('AI'.$x, $nilai_detail['detail']['post']['kolega']['nilai_s_average']);
				$sheet->setCellValue('AJ'.$x, $nilai_detail['detail']['post']['kolega']['nilai_a_average']);
				$sheet->setCellValue('AK'.$x, $nilai_detail['detail']['post']['kolega']['nilai_ksa_average']);
				$sheet->setCellValue('AL'.$x, $nilai_detail['detail']['post']['kolega']['nilai_ksa_x_bobot']);
				$sheet->setCellValue('AM'.$x, $nilai_detail['detail']['post']['kolega']['nilai_b_average']);
				$sheet->setCellValue('AN'.$x, $nilai_detail['detail']['post']['kolega']['nilai_b_x_bobot']);
				$sheet->setCellValue('AO'.$x, $nilai_detail['detail']['post']['kolega']['nilai_x_bobot']);
			}
		}
		
		// Set Border
        $sheet->getStyle ('A1:AO'.$x)->getBorders()->getAllBorders()->setBorderStyle("thin");
