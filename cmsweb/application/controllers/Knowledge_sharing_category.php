...
class Knowledge_sharing_category extends CI_Controller {
    ...
    function ajax_search(){
        ...
        $search_knowledge_sharing_category = $this->category_model->search($query,75);

        if ($search_knowledge_sharing_category!=FALSE) {
            $data_response = array();
            foreach ($search_knowledge_sharing_category as $k => $v ) {
    ...
    function index(){
        has_access('knowledgemanagementcat.view');

        $data['list_menu'] = $this->category_model->get_category_tree($this->section_id,'nama_asc');

        $data['page_name']          = 'Kategori Knowledge Management';
        $data['page_sub_name']      = 'List Kategori';
        $data['is_content_header']  = TRUE;
        $data['page']               = 'knowledge_sharing_category/knowledge_sharing_category_list_view';
        $this->load->view('main_view',$data);
    }

    function create(){
        has_access('knowledgemanagementcat.create');

        $url_return = $this->input->get('url_return');
        if(empty($url_return)){
            $url_return = site_url('knowledge_sharing_category');
        }
        ...
        if ($this->form_validation->run() == FALSE){
            ...
            $data['form_opt_knowledge_sharing_category'][''] = '-';
            $param_query['filter_active'] = '';
            $param_query['filter_section'] = $this->section_id;
            $get_knowledge_sharing_category = $this->category_model->get_all(NULL,NULL,NULL,$param_query);
            if ($get_knowledge_sharing_category!=FALSE){
                foreach ($get_knowledge_sharing_category['data'] as $k => $v) {
                    $data['form_opt_knowledge_sharing_category'][$v['cat_id']] = $v['cat_name'];
                }
            }
            ...
            $data['form_action']    = site_url('knowledge_sharing_category/create').'?url_return='.$url_return;
            $data['page']           = 'knowledge_sharing_category/knowledge_sharing_category_form_create_view';
    ...
    function edit($knowledge_sharing_category_id=NULL){
        has_access('knowledgemanagementcat.edit');

        $knowledge_sharing_category = getCategory($knowledge_sharing_category_id,$this->section_id);
        if ($knowledge_sharing_category['section_id']!=$this->section_id){
            redirect(404);
        }
        ...
        if ($this->form_validation->run() == FALSE){
            ...
            /*Start Select Tree*/
            $data['opt_cat'] = $this->category_model->get_category_tree($this->section_id);
            $cat_selected = $knowledge_sharing_category['cat_parent'];
            ...
            $data['knowledge_sharing_category']      = $knowledge_sharing_category;
            $data['request']            = $knowledge_sharing_category;
            $data['url_return']         = $url_return;
            $data['form_action']        = site_url('knowledge_sharing_category/edit').'/'.$knowledge_sharing_category_id;
            $data['editable']           = TRUE;
            $data['page_name']          = "Kategori Knowledge Management";
            $data['page_sub_name']      = 'Edit Kategori';
            $data['page']               = 'knowledge_sharing_category/knowledge_sharing_category_form_edit_view';
            $this->load->view('main_view',$data);
        }else{
            $post = $this->input->post();
            $data = array(
            ...
            if ($edit==TRUE) {
                create_log($this->section_id,$knowledge_sharing_category_id,'Edit','Kategori');
                flash_notif_success(NULL,$url_return);
            }else{
    ...
    function edit_picture($knowledge_sharing_category_id=NULL){
        has_access('knowledgemanagementcat.edit');
        ...
        if ($this->form_validation->run() == FALSE){
            $knowledge_sharing_category['media_value'] = $knowledge_sharing_category['cat_image'];
            $data['request']            = $knowledge_sharing_category;
            $data['knowledge_sharing_category']            = $knowledge_sharing_category;
            $data['url_return']         = $url_return;
            $data['form_action']        = site_url('knowledge_sharing_category/edit_picture').'/'.$knowledge_sharing_category_id;
            $data['editable']           = TRUE;
            $data['page_name']          = "Kategori Forum";
            $data['page_sub_name']      = 'Edit Gambar'.'<br><small>'.$knowledge_sharing_category['cat_name'].'</small>';
            $data['page']               = 'knowledge_sharing_category/knowledge_sharing_category_form_edit_picture_view';
            $this->load->view('main_view',$data);
        }else{
            $post = $this->input->post();
            ...
            if (isset($_FILES['file']['name'])){
                ...
                if ($this->upload->do_upload('file')){
                    ...
                    $this->category_model->update(
                        array(
                            'cat_id'      => $knowledge_sharing_category['cat_id'],
                            'cat_image'   => $filename_system,
                        )
                    );
    ...
    function delete($knowledge_sharing_category_id=NULL){
        has_access('knowledgemanagementcat.delete');
        ...
        if ($this->form_validation->run() == FALSE){
            ...
            $data['request']            = $knowledge_sharing_category;
            $data['url_return']         = $url_return;
            $data['form_action']        = site_url('knowledge_sharing_category/delete').'/'.$knowledge_sharing_category_id;
            $data['editable']           = TRUE;
            $data['page_name']          = "Kategori Knowledge Management";
            $data['page_sub_name']      = 'Hapus Kategori';
            $data['page']               = 'knowledge_sharing_category/knowledge_sharing_category_form_delete_view';
            $this->load->view('main_view',$data);
        }else{

            $delete = $this->category_model->delete($knowledge_sharing_category_id,FALSE);   //FALSE = Hard Delete
            if ($delete==TRUE) {
                create_log($this->section_id,$knowledge_sharing_category_id,'Hapus','Kategori');
                $msg        = $knowledge_sharing_category['cat_name']." telah dihapus.";
                $url_return = site_url('knowledge_sharing_category');
                flash_notif_warning($msg,$url_return);
            }else{
                $msg        = "Data gagal dihapus.";
                $url_return = site_url('knowledge_sharing_category/delete/'.$knowledge_sharing_category_id);
