...
class Culture_soal_model extends CI_Model {
    ...
    function get_all($keyword=NULL,$limit=NULL,$offset=NULL,$param_query=NULL){
        $this->db->select('SQL_CALC_FOUND_ROWS _culture_soal.*, _category.cat_name,_category.cat_id',FALSE);
        ...
        if (isset($param_query['filter_status'])) {
            $this->db->where('_culture_soal.crs_status',$param_query['filter_status']);
        }

        if (isset($param_query['filter_ids'])) {
            $this->db->where_in('_culture_soal.crs_id',$param_query['filter_ids']);
        }
