...
class Digital_library_category extends CI_Controller {
    ...
    function ajax_search(){
        ...
        $search_digital_library_category = $this->category_model->search($query,75);

        if ($search_digital_library_category!=FALSE) {
            $data_response = array();
            foreach ($search_digital_library_category as $k => $v ) {
    ...
    function index(){
        has_access('digitallibrarycat.view');
        $data['list_menu'] = $this->category_model->get_category_tree($this->section_id,'nama_asc');

        $data['page_name']          = 'Kategori Digital Library';
        $data['page_sub_name']      = 'List Kategori';
        $data['is_content_header']  = TRUE;
        $data['page']               = 'digital_library_category/digital_library_category_list_view';
        $this->load->view('main_view',$data);
    }

    function create(){
        has_access('digitallibrarycat.create');
        $url_return = $this->input->get('url_return');
        if(empty($url_return)){
            $url_return = site_url('digital_library_category');
        }
        ...
        if ($this->form_validation->run() == FALSE){
            ...
            $data['form_opt_digital_library_category'][''] = '-';
            $param_query['filter_active'] = '';
            $param_query['filter_section'] = $this->section_id;
            $get_digital_library_category = $this->category_model->get_all(NULL,NULL,NULL,$param_query);
            if ($get_digital_library_category!=FALSE){
                foreach ($get_digital_library_category['data'] as $k => $v) {
                    $data['form_opt_digital_library_category'][$v['cat_id']] = $v['cat_name'];
                }
            }
            ...
            $data['form_action']    = site_url('digital_library_category/create').'?url_return='.$url_return;
            $data['page']           = 'digital_library_category/digital_library_category_form_create_view';
    ...
    function edit($digital_library_category_id=NULL){
        has_access('digitallibrarycat.edit');

        $digital_library_category = getCategory($digital_library_category_id,$this->section_id);
        if ($digital_library_category['section_id']!=$this->section_id){
            redirect(404);
        }
        ...
        if ($this->form_validation->run() == FALSE){
            ...
            /*Start Select Tree*/
            $data['opt_cat'] = $this->category_model->get_category_tree($this->section_id);
            $cat_selected = $digital_library_category['cat_parent'];
            ...
            $data['digital_library_category']      = $digital_library_category;
            $data['request']            = $digital_library_category;
            $data['url_return']         = $url_return;
            $data['form_action']        = site_url('digital_library_category/edit').'/'.$digital_library_category_id;
            $data['editable']           = TRUE;
            $data['page_name']          = "Kategori Digital Library";
            $data['page_sub_name']      = 'Edit Kategori';
            $data['page']               = 'digital_library_category/digital_library_category_form_edit_view';
            $this->load->view('main_view',$data);
        }else{
            $post = $this->input->post();
            $data = array(
            ...
            if ($edit==TRUE) {
                create_log($this->section_id,$digital_library_category_id,'Edit','Kategori');
                flash_notif_success(NULL,$url_return);
            }else{
    ...
    function edit_picture($digital_library_category_id=NULL){
        has_access('digitallibrarycat.edit');
        ...
        if ($this->form_validation->run() == FALSE){
            $digital_library_category['media_value'] = $digital_library_category['cat_image'];
            $data['request']            = $digital_library_category;
            $data['digital_library_category']            = $digital_library_category;
            $data['url_return']         = $url_return;
            $data['form_action']        = site_url('digital_library_category/edit_picture').'/'.$digital_library_category_id;
            $data['editable']           = TRUE;
            $data['page_name']          = "Kategori Digital Library";
            $data['page_sub_name']      = 'Edit Gambar'.'<br><small>'.$digital_library_category['cat_name'].'</small>';
            $data['page']               = 'digital_library_category/digital_library_category_form_edit_picture_view';
            $this->load->view('main_view',$data);
        }else{
            $post = $this->input->post();
            ...
            if (isset($_FILES['file']['name'])){
                ...
                if ($this->upload->do_upload('file')){
                    ...
                    $this->category_model->update(
                        array(
                            'cat_id'      => $digital_library_category['cat_id'],
                            'cat_image'   => $filename_system,
                        )
                    );
    ...
    function delete($digital_library_category_id=NULL){
        has_access('digitallibrarycat.delete');
        ...
        if ($this->form_validation->run() == FALSE){
            ...
            $data['request']            = $digital_library_category;
            $data['url_return']         = $url_return;
            $data['form_action']        = site_url('digital_library_category/delete').'/'.$digital_library_category_id;
            $data['editable']           = TRUE;
            $data['page_name']          = "Kategori Digital Library";
            $data['page_sub_name']      = 'Hapus Kategori';
            $data['page']               = 'digital_library_category/digital_library_category_form_delete_view';
            $this->load->view('main_view',$data);
        }else{

            $delete = $this->category_model->delete($digital_library_category_id,FALSE);   //FALSE = Hard Delete
            if ($delete==TRUE) {
                create_log($this->section_id,$digital_library_category_id,'Hapus','Kategori');
                $msg        = $digital_library_category['cat_name']." telah dihapus.";
                $url_return = site_url('digital_library_category');
                flash_notif_warning($msg,$url_return);
            }else{
                $msg        = "Data gagal dihapus.";
                $url_return = site_url('digital_library_category/delete/'.$digital_library_category_id);
