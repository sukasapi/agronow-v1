                        ...
                        <div class="row">
							<div class="col-12 mb-2">
                                ...
                                <select class="form-control kt-input" name="cr_kelola" id="cr_kelola" required>
                                    <option value=""></option>
									<option value="dalam_app" <?=($request['cr_kelola']=="dalam_app")?'selected':''?> >dikelola di dalam AgroNow (AgroWallet dan Non AgroWallet)</option>
									<option value="lms_ext_agrowallet" <?=($request['cr_kelola']=="lms_ext_agrowallet")?'selected':''?>>dikelola di luar AgroNow (AgroWallet)</option>
									<option value="luar_app" <?=($request['cr_kelola']=="luar_app")?'selected':''?> >dikelola di luar AgroNow (100% peserta Dari Luar PTPN Group)</option>
                                </select>
							</div>
                    ...
                    <div class="kt-portlet__body">
                        <div class="row">
                            ...
                            <div class="col-12 col-xs-12 text-center">   
                                <?php $pin=$classroom['cr_pin']!=""?$classroom['cr_pin']:$pin;?>
        ...
        var url='<?=base_url('classroom/')?>';  
            ...
            $("input[name=ev_narsum]").on("change",function(e){
                ...
                if(evaluasi == 1){
                    ...
                    var field ="<label class='mt-3 label'>Pengajar <small><i>(* ketikkan nama narasumber kelas)</i></small></label>";
                    ...
                    */      
                    Swal.fire({
                        type: "info",
                        title: "Pemberitahuan",
                        text: "Jika NPS telah terisi oleh peserta, maka data NPS akan terhapus",
                        });
                }
            // console.log(evaluasi)
            })

            $("input[name=ev_penyelenggaraan]").on("change",function(e){
                e.preventDefault();
                var evaluasi=$("input[name=ev_penyelenggaraan]:checked").val();
                if(evaluasi == 1){
            ...
            $("input[name=ev_sarana]").on("change",function(e){
                e.preventDefault();
                var evaluasi=$("input[name=ev_sarana]:checked").val();
                if(evaluasi == 1){
            ...
            $("#addnarsum").on('click',function(e){
                e.preventDefault();
                var field ="<label class='mt-3 labelnew'>Pengajar <small><i>(* ketikkan nama narasumber kelas)</i></small></label>";
                    field +="<div class='input-group mb-3'>";
                    field +="<input class='form-control' name='narsum[]'>";
                    field +="<div class='input-group-append'>";
                    field +="<button class='btn btn-danger btn-small batal' type='button'>X</button>";
            ...
            $(".bthapus").on("click",function(e){
                ...
                hapusevaluasi(kode);
                $(this).closest('.exist_narsum').remove();
