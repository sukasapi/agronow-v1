<script>
    function get_list(page) {
        $('.loading').show();
        $.ajax({
            url: "<?= site_url('whatsnew/bod_share/ajax_get_bod_share')?>?page="+page,
            type: "get",
            dataType: 'json',
                        ...
                        if (contents[i].image){
                            ...
                            html += '<div class="text-left">'+
                                    ...
                                    '<div class="card-footer text-muted">'+
                                        ...
                                        '</div>'+
                                    '<div class="text-left">'+
                                        '<a href="<?=base_url("whatsnew/bod_share/detail/")?>'+contents[i].id+'" class="btn btn-success btn-sm mr-1">SELENGKAPNYA</a>'+
                                    '</div>'+
                                '</div>'+
                                '</div>'+
                                '</div>'+
    ...
    function get_my_list(page) {
        $.ajax({
            url: "<?= site_url('whatsnew/bod_share/ajax_get_my_bod_share')?>?page="+page,
            type: "get",
            dataType: 'json',
            success: function (data) {
                var i;
                ...
                if (contents.length >= 1){
                    for (i=0; i<contents.length; i++){
                        ...
                        html += '<div class="text-left">'+
                            ...
                            '<div>'+
                            '<span class="iconedbox iconedbox-sm" style="width: 60px;">'+
                            '<ion-icon style="content: url(\'<?=PATH_ASSETS?>icon/love-black.png\')"></ion-icon>'+
                            '<span style="font-size: 15px;">&nbsp;'+contents[i].like_count+'</span>'+
                            '</span>'+
                            '</div>'+
                            '<div>'+
                            ...
                            if (contents[i].status === 'draft'){
                                html += '<a href="<?=base_url("whatsnew/bod_share/edit/")?>'+contents[i].id+'" class="btn btn-outline-secondary btn-sm mr-1">DRAFT</a>';
                            } else {
                                html += '<a href="<?=base_url("whatsnew/bod_share/edit/")?>'+contents[i].id+'" class="btn btn-outline-primary btn-sm mr-1">PUBLISHED</a>';
                            }
                            html += '<a href="<?=base_url("whatsnew/bod_share/detail/")?>'+contents[i].id+'" class="btn btn-success btn-sm mr-1">SELENGKAPNYA</a>'+
                            '</div>'+
                            '</div>'+
                            '</div>'+
                            '</div>'+
