...
class Kompetensi_category extends CI_Controller {
    ...
    function ajax_search(){
        ...
        $search_kompetensi_category = $this->category_model->search($query,$this->section_id,75);

        if ($search_kompetensi_category!=FALSE) {
            $data_response = array();
            foreach ($search_kompetensi_category as $k => $v ) {
    ...
    function index(){
        has_access('kompetensicat.view');

        $data['list_menu'] = $this->category_model->get_category_tree($this->section_id);

        $data['page_name']          = 'Kategori Kompetensi';
        $data['page_sub_name']      = 'List Kategori';
        $data['is_content_header']  = TRUE;
        $data['page']               = 'kompetensi_category/kompetensi_category_list_view';
        $this->load->view('main_view',$data);
    }

    function create(){
        has_access('kompetensicat.create');

        $url_return = $this->input->get('url_return');
        if(empty($url_return)){
            $url_return = site_url('kompetensi_category');
        }
        ...
        if ($this->form_validation->run() == FALSE){
            ...
            $data['form_opt_kompetensi_category'][''] = '-';
            $param_query['filter_active'] = '';
            $param_query['filter_section'] = $this->section_id;
            $get_kompetensi_category = $this->category_model->get_all(NULL,NULL,NULL,$param_query);
            if ($get_kompetensi_category!=FALSE){
                foreach ($get_kompetensi_category['data'] as $k => $v) {
                    $data['form_opt_kompetensi_category'][$v['cat_id']] = $v['cat_name'];
                }
            }
            ...
            $data['form_action']    = site_url('kompetensi_category/create').'?url_return='.$url_return;
            $data['page']           = 'kompetensi_category/kompetensi_category_form_create_view';
    ...
    function edit($kompetensi_category_id=NULL){
        has_access('kompetensicat.edit');

        $kompetensi_category = getCategory($kompetensi_category_id,$this->section_id);
        if ($kompetensi_category['section_id']!=$this->section_id){
            redirect(404);
        }
        ...
        if ($this->form_validation->run() == FALSE){
            ...
            /*Start Select Tree*/
            $data['opt_cat'] = $this->category_model->get_category_tree($this->section_id);
            $cat_selected = $kompetensi_category['cat_parent'];
            ...
            $data['kompetensi_category']      = $kompetensi_category;
            $data['request']            = $kompetensi_category;
            $data['url_return']         = $url_return;
            $data['form_action']        = site_url('kompetensi_category/edit').'/'.$kompetensi_category_id;
            $data['editable']           = TRUE;
            $data['page_name']          = "Kategori Kompetensi";
            $data['page_sub_name']      = 'Edit Kategori';
            $data['page']               = 'kompetensi_category/kompetensi_category_form_edit_view';
            $this->load->view('main_view',$data);
        }else{
            $post = $this->input->post();
            $data = array(
            ...
            if ($edit==TRUE) {
                create_log($this->section_id,$kompetensi_category_id,'Edit','Kategori');
                flash_notif_success(NULL,$url_return);
            }else{
    ...
    function delete($kompetensi_category_id=NULL){
        has_access('kompetensicat.delete');
        ...
        if ($this->form_validation->run() == FALSE){
            ...
            $data['request']            = $kompetensi_category;
            $data['url_return']         = $url_return;
            $data['form_action']        = site_url('kompetensi_category/delete').'/'.$kompetensi_category_id;
            $data['editable']           = TRUE;
            $data['page_name']          = "Kategori Kompetensi";
            $data['page_sub_name']      = 'Hapus Kategori';
            $data['page']               = 'kompetensi_category/kompetensi_category_form_delete_view';
            $this->load->view('main_view',$data);
        }else{

            $delete = $this->category_model->delete($kompetensi_category_id,FALSE);   //FALSE = Hard Delete
            if ($delete==TRUE) {
                create_log($this->section_id,$kompetensi_category_id,'Hapus','Kategori');
                $msg        = $kompetensi_category['cat_name']." telah dihapus.";
                $url_return = site_url('kompetensi_category');
                flash_notif_warning($msg,$url_return);
            }else{
                $msg        = "Data gagal dihapus.";
                $url_return = site_url('kompetensi_category/delete/'.$kompetensi_category_id);
