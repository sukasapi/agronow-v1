 ...
 */
class Bod_share extends MX_Controller {
	function __construct()
    {
		...
		$this->load->library('function_api', 'form_validation');
	...
	function index(){
        ...
        $this->data['title'] = 'BOD Share';
        $this->page = 'bod_share';
        $this->menu = 'whatsnew';

        $this->data['bod_share_status'] = false;
        if(@$member['member_ceo'] == '2'){
            $this->data['bod_share_status'] = true;
        }

        $this->customjs = array('bod_share');

        $this->generate_layout();
	}

	function detail($content_id){
        ...
        $content['content_date']    = $this->function_api->date_indo($content['content_publish_date'],"dd FF YYYY");
        $this->data['content']      = $content;

		$this->data['title'] = 'BOD Share';
		$this->page = 'bod_share_detail';
		$this->menu = 'whatsnew';
    ...
    function edit($content_id){
        ...
        if ($this->form_validation->run() == FALSE) {
            $content['image']           = $this->media_model->get_primary_image($content['section_id'],$content['content_id']);
            $this->data['content']  = $content;
            $this->data['url_return'] = site_url('whatsnew/bod_share/detail/'.$content_id);
            $this->data['form_action'] = site_url('whatsnew/bod_share/edit');
            $this->data['title'] = 'CEO Note';
            $this->page = 'bod_share_edit';
            $this->menu = 'whatsnew';
            ...
            redirect(site_url('whatsnew/bod_share/detail/').$content_id);
        }
    }
	...
	function add(){
        ...
        if(empty($url_return)){
            $url_return = site_url('whatsnew/bod_share');
        }
        ...
        if ($this->form_validation->run() == FALSE){
            ...
            $this->data['form_action']    = site_url('whatsnew/bod_share/add');
            $this->data['title'] = 'BOD Share';
            $this->page = 'bod_share_add';
            $this->menu = 'whatsnew';
            $this->customjs = array('ckeditor');
            $this->generate_layout();
            ...
            if(isset($_FILES["content_image1"]) && $_FILES["content_image1"]['name']!=""){
                ...
                $arrName = explode(".",$name);
                $ext = end($arrName);
                $this->media_model->mediaValue 	= "bod_share_image1_".uniqid().'.'.$ext;
                $this->media_model->mediaDesc = "";
                $this->media_model->mediaPrimary= "1";
                $this->media_model->insert_media();
            ...
            if ($this->content_model->lastInsertId){
                redirect(site_url('whatsnew/bod_share/detail/').$this->content_model->lastInsertId);
            }
    ...
    function ajax_get_bod_share(){
        ...
        for($i=0;$i<$d_count;$i++){
            ...
            if(isset($dataContent[$i])){
                ...
                if($author_data && $author_data['member_image']!=""){
                    $tmp['author_image'] = $author_data['member_image'];
                } else {
    ...
    function ajax_get_my_bod_share(){
        $page       = $this->input->get('page');
        $limit      = 5;
        $this->content_model->beginRec = $limit*$page-$limit;
        $this->content_model->endRec = $limit+1;
        ...
        $this->content_model->recData['memberId'] = $this->member_id;
        $dataContent = $this->content_model->select_content("BodShareMyList","", "", $this->mlevel_id, $this->bidang);
        $d_count     = count($dataContent);
        $tmp = [];
        $my_result = [];
        for($i=0;$i<$d_count;$i++){
