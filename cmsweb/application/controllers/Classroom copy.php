    ...
    function create(){
        ...
        if ($this->form_validation->run() == FALSE){
			...
			$data = array(
                ...
                'cat_id'            => $post['cat_id'],
				'id_lw_classroom'   => isset($post['id_lw_classroom'])?(int) $post['id_lw_classroom']:NULL,
				'id_superapp_manpro'   => isset($post['id_superapp_manpro']) && $post['id_superapp_manpro']!=""?(int) $post['id_superapp_manpro']:"0",
				'kode_superapp_manpro' => isset($post['kode_superapp_manpro']) && $post['kode_superapp_manpro']!=""?$post['kode_superapp_manpro']:"0",
            ...
            if ($insert==TRUE) {
                ...
                $id_manpro=isset($post['id_superapp_manpro']) && $post['id_superapp_manpro']!=""?(int) $post['id_superapp_manpro']:"0";
                $id_agronow= $insert;
                if(!empty($id_manpro) && $id_manpro > 0){
                     $url=URL_SIP_PROJECT;
                ...
                $jenis='internal';//besok ganti pake post['cr_kelola'];
                /// Create: 18.01.2024
                /// Auth : KDW
                 ///evaluasi kelas
                 if(isset($post['ev_penyelenggaraan']) && $post['ev_penyelenggaraan'] > 0){
                    ...
                    $inputevaluasi=array('cr_id'=> $insert,'jenis'=>'penyelenggaraan','setsoal'=>$setsoal);
                ...
                if(isset($post['ev_sarana']) && $post['ev_sarana'] >0){
                    ...
                    $inputevaluasi=array('cr_id'=> $insert,'jenis'=>'sarana','setsoal'=>$setsoal);
                    $addevaluasi=$this->ce->add_setsoal($inputevaluasi);
                    if($addevaluasi){
                ...
                if(isset($post['ev_narsum']) && $post['ev_narsum'] >0){
                    ...
                    foreach($post['narsum'] as $n){
                        $inputevaluasi=array('cr_id'=> $insert,'jenis'=>'narasumber','pengajar'=>$n,'setsoal'=>$setsoal);
                        $addevaluasi=$this->ce->add_setsoal($inputevaluasi);
                        if($addevaluasi){
            ...
            $edit = $this->classroom_model->update($data);
             ...
             /// Create: 18.01.2024
                 ...
                 if(isset($post['ev_penyelenggaraan']) && $post['ev_penyelenggaraan'] > 0){
                     ...
                     $setsoal=$this->ce->set_soalbyType("penyelenggaraan",$jenis);
                     $dataupdate=array("cr_id"=>$cr_id,"jenis"=>"penyelenggaraan","status"=>"1","setsoal"=>$setsoal);
                 ...
                 if(isset($post['ev_sarana']) && $post['ev_sarana'] > 0){
                     ...
                     $dataupdate=array("cr_id"=>$cr_id,"jenis"=>"sarana","status"=>"1","setsoal"=>$setsoal);
                     $updateevaluasi=$this->ce->edit_setsoal($dataupdate);
                 ...
                 if(isset($post['ev_narsum']) && $post['ev_narsum'] > 0){
                     foreach($post['narsum'] as $n){
                        $setsoal=$this->ce->set_soalbyType("narasumber",$jenis);
                        $dataupdate=array("cr_id"=>$cr_id,"jenis"=>"narasumber","status"=>"1","pengajar"=>$n,"setsoal"=>$setsoal); 
                        $updateevaluasi=$this->ce->edit_setsoal($dataupdate);
...
/*-- End Fodler download --*/
       ...
       /*-- END MEMBER KELAS UPDATE --*/
        ...
        function tesquery(){
            ...
            $query3 ="SELECT m.member_name,m.member_nip, g.group_name,c.cr_name,c.cr_id,cm.crm_step,c.cr_date_start as tanggal_kelas, g.group_name  FROM _classroom_member as cm 
                      ...
                      where c.cr_create_date >= '2023-10-29 00:00:00' 
    ...
    /*-- Evaluasi Kelas --*/
     ...
     function daftar_evaluasi(){
        ...
        $data['evaluasi']=$data;
        $data['tipe']=array('penyelenggaraan','sarana','narasumber');
