...
class Classroom_model extends CI_Model {
    ...
    private function _get_datatables_query()
    {
		...
		$this->db->where('cr_status', 'publish');
    ...
    /* Fungsi untuk mengambil pre dan post kelas */
    // Auth : KDE
    // Date : 24.01.2024
    /* End Fungsi */
    function get_classresult($filter=null){
        ...
        foreach ($row as $r){
            ...
            $preendScore = $ptScore[2] ? str_replace('.',',',number_format($ptScore[2]/$ptScore[1]*100,1))  : '';
            $endScore = $ctScore[2] ? str_replace('.',',',number_format($ctScore[2]/$ctScore[1]*100,1))  : '';
            $res[$r['cr_name']]["peserta"][]=array("cr_id"=>$r['cr_id'],"nip"=>$r['member_nip'],"nama"=>$r['member_name'],"perusahaan"=>$r['group_name']);
