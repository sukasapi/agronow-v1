...
class Classroom_report extends CI_Controller {
    ...
    function laporan_tes_peserta(){
        if(has_access('classroom_report.tes',FALSE)){

        }else{
            redirect(404);
        }
        $filter="";
        $tahun = $this->input->post('tahun')!="" || $this->input->post('tahun')!=null?$this->input->post('tahun'):date('Y');
        if(isset($_POST) && count((array)$_POST) > 0){
            ...
            if($kelas=="all"){
                ...
                if($filter ==""){
                    $filter .="WHERE cm.cr_id='".$kelas."' ";    
                }else{
                    $filter .=" AND cm.cr_id='".$kelas."' ";      
                }
            }
            ...
            if($bulan == "all"){
                $text1 =$tahun."-01-01 00:00:00";
                $text2 =$tahun."-12-31 23:56:00";
                $start = date("Y-m-d H:i:s",strtotime($text1));
                $end = date("Y-m-d H:i:s",strtotime($text2));
            }else{
                $bl = $bulan;
                $text1 =$tahun."-".$bl."-1 00:00:00";
                $text2 =$tahun."-".$bl."-31 23:56:00";
                $start = date("Y-m-d H:i:s",strtotime($text1));
                $end = date("Y-m-d H:i:s",strtotime($text2));
            }
            
            if($filter ==""){
                $filter .="WHERE c.cr_create_date > '".$start."' 
                AND c.cr_create_date <='".$end."'";    
            }else{
                $filter .="AND c.cr_create_date > '".$start."' 
                AND c.cr_create_date <='".$end."'";
            }
            ...
            if($filter ==""){
                $filter .="WHERE c.cr_create_date > '".$text1."' 
                AND c.cr_create_date <='".$text2."'";    
            }else{
                $filter .="AND c.cr_create_date > '".$text1."' 
                AND c.cr_create_date <='".$text2."'";
            }
        
        }

        $datapost=$this->classroom_model->get_posttes($filter);
     
        $data['peserta']=$datapost;
       // $datapost=$this->classroom_model->get_posttes($filter);
        $data['kelas']=$this->classroom_model->get_kelas($tahun);
        $data['kelaspilih']=$this->input->post("kelas");
        $data['bulanpilih']=$this->input->post("bulan");
        $data['tahunpilih']=$this->input->post("tahun");
        
        $data['section_id']         = $this->section_id;
        $data['page_name']          = 'Class Room';
        $data['page_sub_name']      = 'List Class Room';
        $data['is_content_header']  = TRUE;
        $data['page']               = 'classroom/classroom_laporan_tes';
	...
	function evaluasi_lv3_peserta($classroom_id=NULL){
		...
		$sql =
			"select m.member_id, m.member_name, m.member_nip, g.group_name, cm.crm_step 
			 ...
			 where cm.member_id=m.member_id and m.member_status='active' and cm.member_status='1' and cr_id='".$classroom_id."' and g.group_id=m.group_id
	...
	function evaluasi_lv3_pairing_dl($kat=NULL,$did=NULL,$gid=NULL){
		...
		// Set Border
        $sheet->getStyle ('A1:J'.$x)->getBorders()->getAllBorders()->setBorderStyle("thin");
				...
				if ($sheetData){
					...
					foreach($sheetData as $keyData => $valData) {
						...
						foreach($row as $key => $val) {
							...
							$arrD = array();
							$sqlD =
								"select id, id_penilai, status_penilai
...
function tesquery(){
           ...
           foreach ($row as $r){
                ...
                $endScore = $ctScore[2] ? str_replace('.',',',number_format($ctScore[2]/$ctScore[1]*100,1))  : '';
                echo $r['cr_id'].";".$r['member_id'].";".$ctScore[1].";".$ctScore[2].";".$endScore."<br>";
