...
class Reading_room_category extends CI_Controller {
    ...
    function ajax_search(){
        ...
        $search_reading_room_category = $this->category_model->search($query,75);

        if ($search_reading_room_category!=FALSE) {
            $data_response = array();
            foreach ($search_reading_room_category as $k => $v ) {
    ...
    function index(){
        ...
        $data['page_name']          = 'Kategori Reading Room';
        $data['page_sub_name']      = 'List Kategori';
        $data['is_content_header']  = TRUE;
        $data['page']               = 'reading_room_category/reading_room_category_list_view';
    ...
    function create(){
        ...
        if(empty($url_return)){
            $url_return = site_url('reading_room_category');
        }
        ...
        if ($this->form_validation->run() == FALSE){
            ...
            $data['form_opt_reading_room_category'][''] = '-';
            $param_query['filter_active'] = '';
            $param_query['filter_section'] = $this->section_id;
            $get_reading_room_category = $this->category_model->get_all(NULL,NULL,NULL,$param_query);
            if ($get_reading_room_category!=FALSE){
                foreach ($get_reading_room_category['data'] as $k => $v) {
                    $data['form_opt_reading_room_category'][$v['cat_id']] = $v['cat_name'];
                }
            }
            ...
            $data['form_action']    = site_url('reading_room_category/create').'?url_return='.$url_return;
            $data['page']           = 'reading_room_category/reading_room_category_form_create_view';
    ...
    function edit($reading_room_category_id=NULL){
        $reading_room_category = getCategory($reading_room_category_id,$this->section_id);
        if ($reading_room_category['section_id']!=$this->section_id){
            redirect(404);
        }
        ...
        if ($this->form_validation->run() == FALSE){
            ...
            $data['reading_room_category']      = $reading_room_category;
            $data['request']            = $reading_room_category;
            $data['url_return']         = $url_return;
            $data['form_action']        = site_url('reading_room_category/edit').'/'.$reading_room_category_id;
            $data['editable']           = TRUE;
            $data['page_name']          = "Kategori Reading Room";
            $data['page_sub_name']      = 'Edit Kategori';
            $data['page']               = 'reading_room_category/reading_room_category_form_edit_view';
            $this->load->view('main_view',$data);
        }else{
            $post = $this->input->post();
            $data = array(
    ...
    function delete($reading_room_category_id=NULL){
        $reading_room_category = getCategory($reading_room_category_id,$this->section_id);
        if ($reading_room_category['section_id']!=$this->section_id){
            redirect(404);
        }
        ...
        if ($this->form_validation->run() == FALSE){
            ...
            $data['request']            = $reading_room_category;
            $data['url_return']         = $url_return;
            $data['form_action']        = site_url('reading_room_category/delete').'/'.$reading_room_category_id;
            $data['editable']           = TRUE;
            $data['page_name']          = "Kategori Reading Room";
            $data['page_sub_name']      = 'Hapus Kategori';
            $data['page']               = 'reading_room_category/reading_room_category_form_delete_view';
            $this->load->view('main_view',$data);
        }else{

            $delete = $this->category_model->delete($reading_room_category_id,FALSE);   //FALSE = Hard Delete
            if ($delete==TRUE) {
                $msg        = $reading_room_category['cat_name']." telah dihapus.";
                $url_return = site_url('reading_room_category');
                flash_notif_warning($msg,$url_return);
            }else{
                $msg        = "Data gagal dihapus.";
                $url_return = site_url('reading_room_category/delete/'.$reading_room_category_id);
