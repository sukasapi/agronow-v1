...
//            $validDate = (strtotime($this->data['dataCt']['ctStart'])<=strtotime(date('Ymd')) && strtotime($this->data['dataCt']['ctEnd'])>=strtotime(date('Ymd')));
	...
	public function pretest(){
        ...
        if($this->input->post('sendAnswer')){
			...
			if($ansRight >= $this->data['dataPt']['ReqPassed']){
				...
				$step['PT']['ptScore'] = "A-".count($dataSoalPt)."-".$ansRight."-".$ansWrong;
				$step['PT']['ptStatus'] = "2";
				$step['PL']['plStatus'] = "2";
				...
				$step['PT']['ptStatus'] = "2";
				$step['PT']['ptDate'] = date('Y-m-d H:i:s');
				$step['PT']['ptScore'] = "D-".count($dataSoalPt)."-".$ansRight."-".$ansWrong;
	...
	public function competency(){
		...
		if($this->input->post('sendAnswerCompetency')){
			...
			$ctResult = (($ansRight/count($soal)) * 100);
...
//		    $category = $this->category_model->select_category('parent0',$section_id);
	...
	public function evaluasi_lv3_detail($id_pairing){
		...
		$sql =
			"select 
				c.cr_id, c.cr_name, c.cr_date_start, c.cr_date_end, 
				h.deskripsi_pelatihan, h.tujuan_pelatihan, h.tanggal_mulai, h.tanggal_selesai, h.daftar_pertanyaan,
	...
	public function project_assignment_peserta(){
		...
		if($adapa > 0 ){
				...
				//insert pa baru
				$insertdata=array("cr"=>$_GET['cr_id'],"member_id"=>$_SESSION['member_id']);
	...
	public function pa_ajax(){
		...
		switch($act){
			...
			case 'simpanfinal':
				$pa_id=$this->encryption->decrypt($_POST['paid']);
				$draftupdate=array("atasan_id"=>$_POST['atasan'],"pa_jabatan"=>$_POST['jabatan'],"pa_status"=>"final","pa_problem"=>$_POST["problem"],"pa_solution"=>$_POST["solusi"],"pa_timeframe"=>$_POST["timeframe"],"pa_date_change"=>date('Y-m-d H:i:s'));
				$update_pa=$this->pa->update_pa($draftupdate,$pa_id);
				$res=$update_pa;
			break;
			case 'updatetask':
