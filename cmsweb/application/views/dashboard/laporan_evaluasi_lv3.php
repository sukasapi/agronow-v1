					...
					<div class="kt-portlet__body">
						<div class="mb-2 clearfix">
							...
							<div class="float-right">
								<a class="btn btn-success" href="<?=site_url('classroom/evaluasi_lv3_do_export/th/'.$request['tahun_evaluasi'])?>">Unduh Berkas</a>
								<!--<a class="btn btn-success" download="data.xls" id="downloadlink">download file</a>-->
							</div>
						</div>
...
function filterDataTable(id_group, range) {
	...
	if(id_group=="all") {
		...
		datatable.columns(7).search(kword).draw();
	}
};
...
$(document).ready(function(){
	datatable = $('#dt').DataTable({
        ...
        columns: [
			...
			{ data: 'dnilai', title: 'Nilai', type: 'num', textAlign: 'right' },
	...
	Highcharts.chart('chart0', {
		...
		tooltip: {
			pointFormat: '{series.name}: {point.y} orang ({point.percentage:.1f}%)'
		},
		accessibility: {
			point: {
				valueSuffix: '%'
		...
		plotOptions: {
			pie: {
				...
				cursor: 'pointer',
				dataLabels: { enabled: true, format: '{y} orang ({percentage:.1f}%)' },
				showInLegend: true
			}
		},
		series: [{
