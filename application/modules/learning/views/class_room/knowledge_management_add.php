...
<div class="appHeader text-light" style="background: url('<?=PATH_ASSETS?>icon/login_bg.png'); background-repeat: no-repeat;
    ...
    <div id="appCapsule" class="demoPage">
        <div class="section full mt-2 mb-2 ml-2 mr-2">            <div class="alert alert-info mb-1">
                Silahkan anda berbagi pengetahuan yang telah anda dapatkan dari kelas yang telah diikuti.
            </div>
            <?php
            ...
            <div class="wide-block pt-2 pb-2">
                <form action="<?=base_url('learning/class_room/knowledge_management_add?cr_id='.$cr_id)?>" autocomplete="off" class="needs-validation" method="post" accept-charset="utf-8" enctype="multipart/form-data">
                        ...
                        <div class="input-wrapper">
                            ...
                            <select name="kategori" class="form-control" style="width: 100%" required>
                                ...
                                <?php foreach($category as $c): ?>
                                    <?php if(isset($c['child']) && count($c['child'])>0): ?>
                                        <optgroup label="<?=$c['cat_name']?>">
                                            <?php foreach($c['child'] as $s): ?>
                                                <?php if (isset($s['child']) && count($s['child'])>0): ?>
                                                    <option value="<?=$s['cat_id']?>" disabled>- <?=$s['cat_name']?></option>
